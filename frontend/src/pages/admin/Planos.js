...
const empty = { name: '', price: '', durationDays: 30, description: '' };
...
export default function Planos() {
  ...
  const open = (p) => {
    setEditing(p || null);
    setForm(p ? { name: p.name, price: p.price, durationDays: p.durationDays, description: p.description || '' } : empty);
    setModal(true);
  };
    ...
    <div data-testid="admin-planos-page">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {plans.map((p) => (
            <Card key={p.id} className="p-5 fade-up hover:-translate-y-1 transition-transform duration-200" data-testid={`plano-card-${p.id}`}>
              ...
              <p className="text-xs uppercase tracking-[0.2em] text-muted mb-3">a cada {p.durationDays} dias</p>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Editar Plano' : 'Novo Plano'}>
        <form onSubmit={save} className="space-y-4" data-testid="plano-form">
          ...
          <Field label="Preço (R$)"><Input type="number" step="0.01" min="0" value={form.price} onChange={set('price')} required data-testid="plano-price-input" /></Field>
          <Field label="Duração (dias)"><Input type="number" min="1" value={form.durationDays} onChange={set('durationDays')} required data-testid="plano-duration-input" /></Field>
